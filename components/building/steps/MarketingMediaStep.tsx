// components/building/steps/MarketingMediaStep.tsx
"use client"
...
import { Plus, Minus, Image, FileText, Video } from "lucide-react"
...
export function MarketingMediaStep({ 
  ...
  const handleMainImageComplete = (result: UploadedImage | UploadedImage[]) => {
    ...
    onInputChange("mainImage", uploadedImage.url)
  }
  ...
  const handleFloorPlanComplete = (result: UploadedImage | UploadedImage[]) => {
    ...
    onInputChange("floorPlans", [...currentFloorPlans, ...newUrls])
  }
  ...
  const handleImageDelete = (imageUrl: string) => {
    ...
    } else if (formData.floorPlans?.includes(imageUrl)) {
      onInputChange("floorPlans", formData.floorPlans.filter(url => url !== imageUrl))
    }
  }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Main Building Image <span className="text-red-500">*</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <InstantImageUpload
            key={`main-image-${formData.mainImage}`}
            mode="single"
            projectTitle={formData.name || "Building"}
            imageType="cover"
            existingImages={formData.mainImage}
            editMode={mode === "edit"}
            onUploadComplete={handleMainImageComplete}
            onDelete={handleImageDelete}
            title="Main Building Image"
            description="Required: Upload the primary image that will represent this building"
            maxSize={10}
          ...
          {showGallery && (
            <div className="pt-4 border-t">
              <InstantImageUpload
                ...
                description="Upload multiple images showcasing different areas and features of the building"
                maxSize={10}
                acceptedTypes={['image/jpeg', 'image/jpg', 'image/png', 'image/webp']}
              />
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              <Label className="text-sm font-medium">Do you want to upload floor plans?</Label>
            </div>
            <RadioGroup
              value={showFloorPlans ? "yes" : "no"}
              onValueChange={(value) => setShowFloorPlans(value === "yes")}
              className="flex items-center space-x-6"
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="no" id="floorplan-no" />
                <Label htmlFor="floorplan-no" className="text-sm">No (Skip floor plans)</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="yes" id="floorplan-yes" />
                <Label htmlFor="floorplan-yes" className="text-sm">Yes (Upload floor plans)</Label>
              </div>
            </RadioGroup>
          </div>
          ...
          {showFloorPlans && (
            <div className="pt-4 border-t">
              <InstantImageUpload
                ...
                projectTitle={formData.name || "Building"}
                imageType="cover"
                existingImages={formData.floorPlans}
                editMode={mode === "edit"}
                onUploadComplete={handleFloorPlanComplete}
                onDelete={handleImageDelete}
                title="Floor Plans"
                description="Upload building floor plans, layouts, and architectural drawings"
                maxSize={10}
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <ValidatedInput
              ...
              description="Link to downloadable brochure"
            />
            ...
            <ValidatedInput
              ...
              description="Link to video tour"
            />
          </div>
          ...
          <ValidatedInput
            ...
            description="Link to 3D virtual tour or 360° walkthrough"
          />
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" onClick={handleAddInvestmentHighlight}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            Key Selling Points
            <Button type="button" variant="outline" size="sm" onClick={handleAddSellingPoint}>
              <Plus className="h-4 w-4 mr-1" />
              Add Selling Point
            </Button>
          </CardTitle>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <ValidatedInput
            ...
            setErrors={setErrors}
            type="url"
            placeholder="https://dataroom.company.com/building123"
            maxLength={500}
          />
