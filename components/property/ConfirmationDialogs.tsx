// components/property/ConfirmationDialogs.tsx
"use client"
...
import { savePropertyFormDraft, clearPropertyFormDraft } from "@/lib/property-form-persistence"
...
export function ConfirmationDialogs({ mode, hasUnsavedChanges, formData, onClose, isSubmitting, onResetForm, isModalOpen }: ConfirmationDialogsProps) {
  ...
  const handleSaveDraft = () => {
    if (mode === "add") {
      try {
        savePropertyFormDraft(formData)
  ...
  useEffect(() => {
    ...
    ;(window as any).handlePropertyFormClose = handleCloseAttemptFromParent
    
    return () => {
      delete (window as any).handlePropertyFormClose
    }
